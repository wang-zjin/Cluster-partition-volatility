...
function results_HAR_a = VolFcst_HAR_a(Read_Name,Time_Outsample,varargin)
parseObj = inputParser;
functionName='VolFcst_HAR-a';
...
vf_HAR_a=zeros(n,1);
% in-sample fitness
ismae_HAR_a=zeros(n,1);
isrmse_HAR_a=zeros(n,1);
% dynamic parameter 
para_HAR_a=zeros(n,7);
% initial setting
...
for i=I(1):I(end)
    ...
    vf_HAR_a(i1)=exp([1,vt_1(end),vt_5(end),vt_22(end),Jump(end),a1(end),a2(end)]*paras);
    para_HAR_a(i1,:)=paras;
    ismae_HAR_a(i1)=mean(abs(v_HAR-ret.^2));
    isrmse_HAR_a(i1)=sqrt(mean((v_HAR-ret.^2).^2));
end

results_HAR_a.vf_HAR_a=vf_HAR_a;
results_HAR_a.para_HAR_a=para_HAR_a;
results_HAR_a.ismae_HAR_a=ismae_HAR_a;
results_HAR_a.isrmse_HAR_a=isrmse_HAR_a;
